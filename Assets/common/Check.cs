...
public class Check {
    ...
    /// </summary>
    /// <returns></returns>
    public static bool CheckPassWold(string str)
    {
        //密码(以字母开头，长度在6~18之间，只能包含字母、数字和下划线)
        //Regex reg = new Regex(@"^[a-zA-Z]\w{5,17}$");
        Regex reg = new Regex(@"^[a-zA-Z][a-zA-Z0-9_]{5,17}$");
    ...
    {
        //Regex reg = new Regex(@"^[a-zA-Z]\w{5,13}$");
        Regex reg = new Regex(@"^[a-zA-Z][a-zA-Z0-9_]{5,13}$");
        return reg.IsMatch(str);
    }
    /// <summary>
    /// 检查验证吗
    /// </summary>
    /// <returns></returns>
    public static bool CheckValidCode(string str)
    {
        Regex reg = new Regex(@"^[0-9]{4,4}$");
        return reg.IsMatch(str);
    }
    public static bool CheckPhoneNum(string str)
    {
        //手机号码
        if (str.Length != 11)
            return false;
        Regex reg = new Regex(@"^1[0-9]\d{9}$");
        return reg.IsMatch(str);
    }
    public static bool CheckGonghaoNum(string str)
    {
        //手机号码
        if (str.Length != 8)
            return false;
        Regex reg = new Regex(@"^201\d{5}$");
        return reg.IsMatch(str);
    }
     public static bool IsValidIP(string ip)
    {
        // 这是一个简单的正则表达式，它可能不会匹配所有的IP地址格式，但可以匹配大多数常见的格式。  
        // 对于更全面的检查，您可能需要使用更复杂的正则表达式或库函数。  
    ...
    {
        return Regex.IsMatch(num, @"^[1-9]d*$");
    }
}
